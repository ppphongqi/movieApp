...
<template>
	<view>
		...
		<block v-if="viewStyle === '1'">
			<view class="share-content" :class="isShare?'isshare':'isshareFlase'">
				<!-- 商品海报 -->
				<view class="poster-box" @click="shareswich">
					<block v-if="isShowPoster && isShare">
						<image :src="posterImages" mode="aspectFit" class="poster-show" v-if="posterImages"></image>
						<view class="poster-image" v-if="!posterImages">
							<share-poster :poster-type="posterType" :poster-data="{pId:pid,activitysId:activitysId}" @success="posterImages = $event"></share-poster>
						</view>
					</block>
				</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			isShowPoster() { //是否直接显示
				return ["goods", "dealer", "member"].includes(this.posterType);
			}
		},
		mounted() {
			// #ifdef H5
		...
		methods: {
			...
			updateShareGoods() {
				...
				if (clientType === "2") {
					...
					path = App.siteInfo.siteroot + "h5/#/pages/subPages/goods/index/index?" + App.urlEncode(
						activityData);
				}
