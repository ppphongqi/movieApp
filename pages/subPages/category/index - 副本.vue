	...
	<view class="container" :class="tabContainer" :style="themeColor">
		<page-header :content-show="false" comPage="query" backgroundColor="#FFFFFF">
			<block slot="user">
				<view class="index-search-box">
					<navigator class="index-search " hover-class="none" openType="navigate" url="/pages/subPages/category/list">
				...
				<view class="cate-content search-class-list-1">
					<scroll-view scroll-x="true" >
						<view class="main-class dis-flex flex-y-center">
							<view 
							class="main-class-item kind-item t-c dis-flex flex-dir-column flex-x-center flex-y-center" 
							:class="{'active':mainIndex === -1}" 
							@click="filterChange(-1,-1,'main')">
								<view
								class="f-28 m-top20 t-c " 
								>全部分类</view>
							</view>
							<block v-for="(item, mIndex) in list" :key="mIndex">
								<view 
								class="main-class-item kind-item t-c dis-flex flex-dir-column flex-x-center flex-y-center" 
								:class="{'active':mainIndex == mIndex}" 
							...
							<scroll-view scroll-x="true" class="us-look-list">
								<view class="dis-flex flex-y-center small-class-item-box">
									<block v-for="(item,sIndex) in list[mainIndex].child" :key="item.category_id">
										<view 
										class="small-class-item main-class-item-1 kind-item t-c dis-flex flex-dir-column flex-x-center flex-y-center" 
										:class="{'active':small_id === item.category_id}" 
										@click="filterChange(item.category_id,sIndex,'small')">
											<view class="f-24 t-c" > 
												{{item.name}}
											</view>
										</view>
		...
		<view v-if="showProduct" class="search-query-box" :class="isTabarFixed?'fixed':''">
			<view class="dis-flex flex-x-between flex-y-center col-3" >
				<view 
				@click="confirmFilter(item,index)" 
				class="dis-flex flex-x-center flex-y-center f-28 class-list-box" 
				v-for="(item,index) in filterData"
				:key='index' 
				:class="{'filter':item.check || (index === 0 && item.name != '商圈') }">
			...
			<view class="shangq-box" :class="showShangqBox?'active':''" v-show="showShangqBox">
				...
				<ul class="clear" style="padding-left:0;">
					<li class="fl shangq-list num-height-1" 
					:class="shangqListIdx==index?'active':''" 
					@click="changeShangQListIdx(item,index)" 
	...
	export default {
		...
		computed: {
			showProduct(){
				...
				return appSetting.product === '1' ? true : false
			},
			itemStyle() {
				let itemStyle = {
						display: "3",
			...
			goodsListHandle() {
				...
				let _this = this,
					...
					is_goods_long = appSetting.is_goods_long;
				if (_this.goods_list) {
			...
			confirmFilter(item,index){
				...
				if(index === 0){
					...
					if(index===1){
						...
						this.filterData[1].check = this.paramsObj.newest == 1 ? true : false
						this.resetListData()
					}
					// 附近优先
					else if(index===2){
						...
						this.filterData[2].check = this.paramsObj.isNeibor == 1 ? true : false
...
<style lang="scss" scoped="">
	...
	.main-class-item.active view,.main-class-item.allactive view{
		// color: var(--theme-color);
