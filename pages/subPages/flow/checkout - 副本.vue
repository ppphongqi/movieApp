<template>
	<view :style="themeColor">
		<view class="container">
			...
			<view class="page-nav b-f" v-if="options.transport_type == 20 ||(options.transport_type == 60 && options.is_express == 1) || options.transport_type == 50">
				<block v-if="navList.length > 1">
					<page-nav-bar @navItemClick="expressTypeChange" ref="pageNav" :nav-list="navList" current="express"></page-nav-bar>
				</block>
				<!-- 物流配送 -->
				...
				<block v-if="isCart">
					<block v-for="(storeItem,index) in goodsData" :key="index">
						<view class="store-item b-f">
							...
							<block v-for="(c,gIndex) in storeItem.goods_list" :key="goods.goods_id">
			...
			<view class="money-list m-top20">
				...
				<view class="flow-all-money b-f">
					...
					<!-- 快递配送费用 end -->
					<!-- 同城配送费用 -->
					<view class="dis-flex flow-all-list border-line border-bottom flow-border-none" v-if="expressType ==='city' && options.transport_type != 50">
						<text class="flex-five">配送费用</text>
						<view class="flex-five t-r">
							<view v-if="!!address">
	...
	export default {
		...
		computed: {
			...
			cTicketingInfoCard() {
				...
				let visible = false
				if (!this.ticketingUserActiveItem.info_id && this.options.transport_type == 60) {
			...
			cTicketingUserData() {
				...
				console.log('cTicketingUserData', arr)
				return arr
			},
			// 格式化的传给日历价格组件的数据
			cCalendarPriceData() {
			...
			actual_pay_price() {
				...
				// 如果是同城配送
				if(expressType === 'city'  && this.options.transport_type != 50) {
		...
		methods: {
			...
			pageLoad(options) {
				...
				} else if (['60'].indexOf(options.transport_type) !== -1) {
					this.expressType = ''
				} else {
					this.expressType = 'self'
				}
			...
			setExpressList({
				...
				if (transport_type === '20') {
					...
					if (run_errands == '1' && city_delivery_state == '1') {
			...
			paySuccess(e) {
				...
				if (!!this.options.free_id) {
					url =
						`/pages/subPages/myfreeBill/miandiansuccess?goods_id=${this.options.goods_id}&free_id=${this.options.free_id}`
				}
			...
			async submitOrderReal({
				...
				if(expressType === 'express' || expressType === 'city') {
					// 如果是快递配送或者同城配送
					addressData = address
				}else if(isCart) {
					// 如果是购物车进入下单页面
					addressData = goodsData.map(val => !!val.store)
				}else if(transport_type != '10' && transport_type != '50') {
					addressData = store
				}
				...
				if (transport_type === '60') {
					...
					for (let i = 0, len = this.ticketingUserActiveItems.length; i < len; i++) {
						...
						if (!val.info_id || `${val.info_id}`.indexOf('fake') !== -1 || !val.name || !val.phone) {
							App.showError(`请完善票务用户信息(${i + 1}/${len})`)
							return
						}
				...
				this.disabled = true
				const express_type = ((this.navList || []).find(item=>item.id === expressType) || {}).value
				// 创建订单-立即购买
				let payData = {
				...
				if (isCart) {
					...
					goodsData.map(storeItem => {
						...
						if (transport_type != 40) {
							...
							goods_item.addressOrStore = expressType === 'express' ? (addressData ? addressData.address_id : '') : (is_zt ==
