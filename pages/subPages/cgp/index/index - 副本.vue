	...
	<view class="container" :style="themeColor">
		<view style="height: 100rpx;padding: 20rpx;" class="dis-flex flex-x-center  flex-y-center">
			<view class="search-input dis-flex flex-x-center  flex-y-center">
				...
				<input class="input" placeholder="搜索商品" v-model="searchValue" placeholderStyle="color:#aba9a9;font-size:12px;"
							...
							<view class="goodsDetail-list-item p-top-bom-30" v-for="(goodsDetailItem,index) in cGoodsList" :key="index"
							 @click="todetial(goodsDetailItem.goods_id)">
								<view class="dis-flex">
									...
									<view class="goodsDetail-right dis-flex flex-dir-column flex-x-between p-r flex-box">
										...
										<view class="shoppIng-main">
											<view class="col-9 f-24 t-r" style="margin-bottom: auto;">已售{{goodsDetailItem['goods_sales']}}件</view>
											<view v-if="goodsDetailItem.stock_num <= 0" class="shoppIng-main_label-status is-disabled">已售罄</view>
											<view v-else-if="goodsDetailItem.sale_type==0" class="p-r" @click.stop>
												<!-- <image src="/static/images/grouppurchase/shopping.png" mode=""></image>
								 			<view class="cart-num col-f f-24" v-if="goodsDetailItem['cart_num']>0">{{goodsDetailItem['cart_num']}}</view> -->
											...
											<view v-else class="shoppIng-main_label-status">
								...
								<view style="padding-top: 30rpx;">
									<goods-tip v-if="goodsDetailItem.tip" :tip="goodsDetailItem.tip" containerStyle="padding-bottom: 24rpx;" />
								</view>
							</view>
						</view>
		...
		<diy-tab :cNum="cartTotal"></diy-tab>
...
<script>
	...
	import GoodsTip from '../components/GoodsTip.vue'
	...
	export default {
		...
		computed: {
			...
			cOneClassList() {
				return this.oneClassList.map(item => {
					// redude得到徽标数
					const badgeNum = (this.cCartList || []).reduce((total, cartGoods = {}) => {
						if ((cartGoods.category || []).find(id => id === item.category_id)) {
							return total + cartGoods.goods_num
						}
						return total
					}, 0)
					return {
			...
			cCartList() {
				return this.$store.state.cgp.cartGoodsListRes.goods_rows || []
			},
			// 格式化的商品列表数据 要从vuex里面取得购物车数据
			cGoodsList() {
				return this.goodsList.map(item => {
					const cartGoods = this.cCartList.find(cartItem => cartItem.goods_id === item.goods_id)
					return {
						...item,
						cartGoods,
						// 购物车数量
						cart_num: cartGoods ? cartGoods.goods_num : 0,
						// 库存
		...
		onShow() {
			...
			this.$store.dispatch('cgp/fetchCartGoodsListRes')
		},
		onLoad(options) {
		...
		methods: {
			...
			todetial(id) {
				App.navigationTo({
					url: 'pages/subPages/goods/index/index?cgp=1&goods_id=' + id
				})
			},
