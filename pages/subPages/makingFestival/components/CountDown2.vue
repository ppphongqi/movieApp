...
<template>
	<view class="count-down">
		<!-- header -->
		<view class="count-down__header">{{cCountDownData.title}}</view>
		<!-- header end -->
		<!-- content -->
		<view v-if="timer" class="count-down__content">
...
<script>
	...
	export default {
		...
		computed: {
			cCountDownData() {
				let title = '活动倒计时'
				let timeStamp = this.endTimeStamp
				if(this.nowTimeStamp < this.startTimeStamp) {
					title = '距活动开始'
					timeStamp = this.startTimeStamp
				}else if(this.nowTimeStamp > this.endTimeStamp) {
					title = '活动已经结束'
				}
				return {
					title,
		...
		methods: {
			...
			startCountDown() {
				...
				if(+dayjs() > this.cCountDownData.timeStamp) {
					this.endCountDown()
					return
				}
				this.timer = setInterval(()=>{
					this.nowTimeStamp = +dayjs()
					const now = this.nowTimeStamp
					const endTimeStamp = this.cCountDownData.timeStamp
					if(endTimeStamp < now) {
						console.log('倒计时结束啦')
						this.endCountDown()
						this.resetTime()
					...
					const duration = dayjs.duration(endTimeStamp - now)
