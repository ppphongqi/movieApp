...
<template>
	<view>
		<pop-manager :show="showList" type="bottom" @clickmask="clickMask">
			<view class="order-pay-menu" :style="{height:PayMenuHeight+'px',paddingBottom}">
...
// #ifdef MP-WEIXIN
const { scene:wxScene } = wx.getEnterOptionsSync();
// #endif
export default {
    data() {
        return {
    ...
    methods: {
        ...
        onToggleList(callback) {
            if (this.isZeroPay) {
                ...
                console.log('onToggleList:payData', payData)
                ...
                if (!this.showList) {
                    ...
                    } else if (this.radioList && this.radioList.length === 1) {
                        // 只有一种支付方式就直接支付了
                        console.log('只有一种支付方式就直接支付了')
                        this.formSubmit()
                    } else {
                        this.showList = true
                    }
        ...
        formSubmit(e) {
            ...
            case 'live': // 直播申请结算
                ...
                break
            case 'assist': // 票务支付
                requestUrl = 'assist.Assist/payCode'
                break
            case 'apply': // 申请分销支付
                requestUrl = 'user.dealer.Apply/submit'
                break
            case 'cardOrderPay': // 卡包订单支付
                    requestUrl = 'card.card/buyCard'
                    break
            case 'payBill': // 申请分销支付
                requestUrl = 'phoneBill.Index/payBill'
                break
			case 'payOnline': // 在线买单
			    requestUrl = 'payonline.PayOnline/pay'
			    break
			case 'mangopay': // 电影票
			    requestUrl = 'movie.Movie/payMovieOrder'
			    break
            }
            ...
            if (!!paymentUrl && !!currentPayType && currentPayType === payType) {
                ...
                App._post_form(requestUrl, {
                    ...
                    if (_this.isZeroPay) { // 0元支付
                        ...
                        } else if (source === '3' || source === '4') { // H5支付
        ...
        requestPayment(clientType, payInfo) {
            ...
            if (clientType === '2') {
				...
				if(payInfo.data.payment.orderInfo) {
					payRequestData.orderInfo = payInfo.data.payment.orderInfo
				}
				console.log('支付参数',payRequestData)
            }
            // 支付宝
            ...
            console.log(payRequestData)
			if(clientType === '2') {
				console.log('wx.requestOrderPayment')
				wx.requestOrderPayment({
				    ...
				    success(res) {
				        ...
				        App._post_form('user.order/selfCheckOrder', {
				            order_id: payInfo.data.order_id
				        }, res => {})
				    },
				    fail(res) {
				        uni.hideLoading()
