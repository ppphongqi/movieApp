		...
		<view class="navbar-box b-f" style="width: 100%;" :class="{ fixed: fixed == '1' }" :style="fixed == '1' ? fixTopHeight : ''">
			<scroll-view :scroll-x="isScroll" style="width: 100%;" :scroll-into-view="'id'+currentIndex" scroll-with-animation="true">
				<view class="nav-list" :style="navListWidth">
					<view class="nav-items" v-for="(items, index) in newNavList" :key="index" @click="navItemClick(items.id, index,item)" :id="'id'+items.id">
						...
						<view class="select-nav-items" v-if="currentIndex == items.id"><image src="/static/images/br1.png" class=" main-bg-color" mode=""></image></view>
					</view>
				</view>
			</scroll-view>
		</view>
