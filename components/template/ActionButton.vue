<template>
	<view class="page">
		<button
		  class="custom-class theme-class zan-btn"
		  :class="[{'zan-btn--last':isLast}, size ? 'zan-btn--'+ size : '',size === 'mini' ? 'zan-btn--plain' : '', plain ? 'zan-btn--plain' : '',type ? 'zan-btn--'+type : '',loading ? 'zan-btn--loading' : '',disabled ? 'zan-btn--disabled' : '']"
		  :disabled="disabled"
		  :open-type="openType"
		...
		>
			<action-button-text :text="btnText" :loading="loading" :type="btnType"></action-button-text>
		</button>
	</view>
</template>

<script>
	import ActionButtonText from "@/components/template/ActionButtonText.vue"
	export default {
		data() {
			return {
				inGroup: false,
