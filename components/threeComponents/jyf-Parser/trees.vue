<template>
	<view style="display: inherit;white-space: inherit;" class="pares-content">
		<block v-for='(item, index) in nodes' v-bind:key='index'>
			<!--#ifdef MP-WEIXIN || MP-QQ || APP-PLUS-->
			<block v-if="handler.isContinue(item)">
				<!--图片-->
				<rich-text v-if="item.name=='img'" class="img" :style="'text-indent:0;'+handler.getStyle(item.attrs.style,'inline-block')+';max-width:100%!important;'"
			...
			tapEvent(e) {
				...
				if (/^http/.test(e.currentTarget.dataset.href)) {
					if (this._top.autocopy) {
						...
						this.navigationTo({
							type: "12",
							path: e.currentTarget.dataset.href
						})
						// #endif
						// #ifdef H5
