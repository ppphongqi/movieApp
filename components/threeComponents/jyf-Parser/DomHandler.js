...
DomHandler.prototype.onopentag = function(name, attrs) {
	...
	if (attrs.style && attrs.style.indexOf("width") !== -1) {
		let reg = new RegExp("width:(\s)*[0-9]*px"),
			systemInfoSync = uni.getSystemInfoSync(),
			attrsList = attrs.style.split(";").map(val => {
				if (val && val.indexOf(":") !== -1) {
					let [attr, attrValue] = val.split(":");
					if (attr.trim() === "width") {
						let widthNum = Number(attrValue.substring(0, attrValue.length - 2));
						if (!isNaN(widthNum) && widthNum > systemInfoSync.windowWidth) {
							val = "width:100%";
						}
					}
				}
