		...
		<view class="w-picker-cnt" :class="{'show':showPicker}">
			...
			<view class="w-picker-view" v-if="mode=='date'||mode=='dateTime'||mode=='yearMonth'">
				<picker-view :indicator-style="itemHeight" :value="pickVal" @change="bindChange">
					<picker-view-column>
						<view class="w-picker-item" v-for="(item,index) in yearsArr || []" :key="index">{{item}}年</view>
					</picker-view-column>
					<picker-view-column>
						<view class="w-picker-item" v-for="(item,index) in monthsArr || []" :key="index">{{item}}月</view>
					</picker-view-column>
		...
		computed: {
			yearsArr() {
				if (Array.isArray(this.data.years)) {
					return this.reverseTime ? this.data.years.reverse() : this.data.years;
				}
			},
			monthsArr() {
				if (Array.isArray(this.data.months)) {
					const nowMonths = new Date().getMonth() + 1;
					// const scopeMonths = []
					// this.data.months.some(item=>{
					//  const isMax = parseInt(item) > nowMonths
					//  if(!isMax){
					// scopeMonths.push(item)
					// return isMax
					//  }
					// })
					return this.reverseTime ? this.data.months.reverse() : this.data.months;
				}
			}
		},
		props: {
			...
			initData() {
				...
				switch (mode) {
					...
					case "yearMonth":
						let nowTime = new Date(),
							[yearData, monthData] = [data.years, data.months].map((val, index) => {
								let findList = val.join(" ").split(" ").reverse(),
									findValue = String(index === 0 ? nowTime.getFullYear() : nowTime.getMonth() + 1),
									findItem = null;
								findValue = findValue.length === 1 ? "0" + findValue : findValue;
								findItem = {
									index: findList.indexOf(findValue),
									value: findValue
								}
								return findItem
							})
						data.defaultVal = [yearData.index, monthData.index];
						year = yearData.value;
				...
				_this.$nextTick(() => {
					if (data.defaultVal && _this.current) {
						...
						_this.pickVal = mode === "yearMonth" ? data.defaultVal : _this.defaultVal;
					}
				})
			}
		},
