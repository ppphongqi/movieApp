<template>
	<view class="tui-skeleton-cmomon tui-skeleton-box" :style="{width: winWidth+'px', height:winHeight+'px', backgroundColor:backgroundColor}">
		<view class="tui-skeleton-cmomon tsts" v-for="(item,index) in skeletonElements" :key="index" :style="{width: item.width+'px', height:item.height+'px', left: item.left+'px', top: item.top+'px',backgroundColor: skeletonBgColor,borderRadius:getRadius(item.skeletonType,borderRadius)}"></view>
...
<style scoped>
	...
	{
		0%   {background:#e9e9e9;}
		25%  {background:#d1d1d1;}
		50%  {background:#e2e2e3;}
		75%  {background:#ebebec;}
		100% {background:#f0f0f1;}
	}
