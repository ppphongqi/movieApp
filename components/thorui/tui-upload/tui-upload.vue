				...
				uni.chooseImage({
					...
					success: function(e) {
						...
						for (let i = 0; i < e.tempFiles.length; i++) {
							...
							uni.compressImage({
							  src: e.tempFiles[i].path,
							  quality: 80,
							  success: res => {
								imageArr.push(res.tempFilePath)
								_this.imageList.push(res.tempFilePath)
						...
						success: function(res) {
							if (res.statusCode == 200) {
								...
								if (d.success) {
									...
									_this.$set(_this.statusArr, index, d.success ? "1" : "3")
									_this.$set(_this.action, 0, 1)
									for (let o in d) {
										_this.$set(_this.upinfo, o, d[o])
									}
								} else {
									// 上传失败
